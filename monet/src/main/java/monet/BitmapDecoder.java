...
class BitmapDecoder {
  ...
  static Bitmap decode(Request request) throws IOException {
  ...
  private static void calculateInSampleSize(int reqWidth, int reqHeight, int width, int height,
    ...
    if (height > reqHeight || width > reqWidth) {
      ...
      if (reqHeight == 0) {
        ...
        final ImageView view = request.fit();
        if (view != null && view.getScaleType() == ImageView.ScaleType.CENTER_INSIDE) {
